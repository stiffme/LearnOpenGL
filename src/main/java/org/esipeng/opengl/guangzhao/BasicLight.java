...
public class BasicLight extends OGLApplicationGL33 {
    ...
    protected boolean applicationInitAfterContext() {
        try{
            m_programObject = compileAndLinkProgram(
                    "BasicLightExample/vertex.glsl",
                    "BasicLightExample/fragment.glsl"
            );
            m_programLight = compileAndLinkProgram(
                    "BasicLightExample/vertex.glsl",
                    "BasicLightExample/fragmentLight.glsl"
            );
        } catch (Exception e)   {
            e.printStackTrace();
            return false;
...
//        };
        ...
        int blockByteSize = glGetActiveUniformBlocki(m_programObject, viewProjectionLoc, GL_UNIFORM_BLOCK_DATA_SIZE);
        System.out.println("Block size is " + blockByteSize);
    ...
    public static void main(String[] args)  {
        OGLApplicationAbstract application = new BasicLight();
        application.run();
    }
}
