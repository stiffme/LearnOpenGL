...
public class Camera3DExample extends OGLApplicationGL33 {
    int m_width = 800, m_height = 600;
    int m_program, m_vao, m_texture1, m_texture2;
    int  m_modelLoc = -1, m_viewLoc = -1, m_projectionLoc = -1;
    ...
    protected boolean applicationCreateContext() {
        m_window = glfwCreateWindow(m_width, m_height, "CoordSystemExample 3D", NULL, NULL);
        if(m_window == NULL)
            return false;
    ...
    protected boolean applicationInitAfterContext() {
        ...
        m_camera.setSpeedPerMilisecond(5.f);
        float[] vertices = {
        ...
        if(m_projectionLoc == -1)
            return false;
        frameBufferSizeChanged(m_window,m_width, m_height);


        //uniform mat4 view, view will be updated in update(), send to GL in draw();
...
//                0.f,1.f,0.f
//        );
//        glUniformMatrix4fv(m_viewLoc, false, matView.get(new float[16]));
    ...
    protected void draw() {
        ...
        glUniformMatrix4fv(m_viewLoc,false, m_camera.generateView());

        Matrix4f matProjection = new Matrix4f();
        matProjection.setPerspective((float)Math.toRadians(m_camera.getFov()), (float)(m_width / m_height), 0.1f, 100.f);
        glUniformMatrix4fv(m_projectionLoc, false, matProjection.get(new float[16]));
    ...
    public static void main(String[] args)  {
        OGLApplicationAbstract application = new Camera3DExample();
        application.run();
    }
}
