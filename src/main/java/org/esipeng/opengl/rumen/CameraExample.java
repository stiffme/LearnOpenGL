...
public class CameraExample extends OGLApplicationGL33 {
    int m_width = 800, m_height = 600;
    int m_program, m_vao, m_texture1, m_texture2;
    int  m_modelLoc = -1, m_viewLoc = -1;
    float m_radius = 15.f;

    Matrix4f m_model = new Matrix4f(), m_matView = new Matrix4f();
    ...
    protected void update(float elapsed) {
        //update view
        m_matView.identity();
        float x = (float)Math.sin(glfwGetTime() ) * m_radius;
        float z = (float)Math.cos(glfwGetTime() ) * m_radius;
        m_matView.lookAt(
                x, 0.f, z,
                0.f,0.f,0.f,
                0.f, 1.f,0.f
        ...
        glUseProgram(m_program);
        glUniformMatrix4fv(m_viewLoc, false, m_matView.get(m_modelBuf));
    }
    ...
    public static void main(String[] args)  {
        OGLApplicationAbstract application = new CameraExample();
        application.run();
    }
}
