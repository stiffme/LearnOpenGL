...
public class ParallaxMapping extends OGLApplicationGL33 {
    private static final Logger logger = LoggerFactory.getLogger(ParallaxMapping.class);
    ...
    protected boolean applicationInitAfterContext() {
        ...
        try {
            mShader = compileAndLinkProgram(
                    "advanced/ParallaxMapping/vertex.glsl",
                    "advanced/ParallaxMapping/fragment.glsl"
            );

            mDiffuseMap = loadTextureFromResource("advanced/ParallaxMapping/bricks2.jpg");
            mNormalMap = loadTextureFromResource("advanced/ParallaxMapping/bricks2_normal.jpg");
            mDepthMap = loadTextureFromResource("advanced/ParallaxMapping/bricks2_disp.jpg");
        } catch (Exception e)   {
            e.printStackTrace();
            return false;
        }
        ...
        glfwSetKeyCallback(mWindow, new GLFWKeyCallback() {
            ...
            public void invoke(long window, int key, int scancode, int action, int mods) {
                if(key == GLFW_KEY_B && action == GLFW_RELEASE) {
                    ParallaxMapping.this.enableNormalMap = !ParallaxMapping.this.enableNormalMap;
                }
    ...
    public static void main(String[] args)  {
        OGLApplicationAbstract application = new ParallaxMapping();
        application.run();
    }
}
