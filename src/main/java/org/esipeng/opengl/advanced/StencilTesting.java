...
public class StencilTesting extends OGLApplicationGL33 {
    ...
    protected void draw() {
        ...
        glUseProgram(mProgramLining);

        mModel.identity().translate(-1.0f, 0.0f, -1.0f).scale(1.1f);
        mvpManager.setValue("model", mModel);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        mModel.identity().translate(2.0f, 0.0f, 0.0f).scale(1.1f);
        mvpManager.setValue("model", mModel);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        glBindVertexArray(0);
    ...
    public static void main(String[] args)  {
        OGLApplicationAbstract application = new StencilTesting();
        application.run();
    }
}
