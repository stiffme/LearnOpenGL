...
public class BlendTesting2 extends OGLApplicationGL33 {
    private static final int MVP_BINDING_POINT = 5;
    int mWidth = 800, mHeight = 800;
    int mObjectVAO, mPlaneVAO, mProgramObject;
    int mTransparentVAO;
    long mWindow;
    UBOManager mvpManager;
    Camera mCamera;
    Matrix4f mModel, mView, mProjection;
    int textureMetal, textureMarble, textureGrass;

    Vector3f[] vegetations = new Vector3f[] {
    ...
    protected boolean applicationInitAfterContext() {
        try {
            mProgramObject = compileAndLinkProgram(
                    "advanced/blend/vertex.glsl",
                    "advanced/blend/fragment.glsl");
        ...
        int transparentVBO = getManagedVBO();
        ...
        mTransparentVAO = getManagedVAO();
        glBindVertexArray(mTransparentVAO);
        glBindBuffer(GL_ARRAY_BUFFER, transparentVBO);
        glVertexAttribPointer(0,3,GL_FLOAT,false, Float.BYTES * 5, 0L);
        glVertexAttribPointer(1,2,GL_FLOAT,false,Float.BYTES * 5, Float.BYTES * 3);
        glEnableVertexAttribArray(0);
        ...
        try{
            ...
            if(!textureLoader.loadFromResource("advanced/blend/window.png"))
                return false;
            textureGrass = getManagedTexture();
    ...
    protected void draw() {
        ...
        mModel.setTranslation(-1.0f, 0.0f, -1.0f);
        mvpManager.setValue("model", mModel);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        mModel.setTranslation(2.0f, 0.0f, 0.0f);
        mvpManager.setValue("model", mModel);
        glDrawArrays(GL_TRIANGLES, 0, 36);
        ...
        List<Vector3f> sortedVeges = Arrays.asList(vegetations);
        Vector4f temp1 = new Vector4f();
        Vector4f temp2 = new Vector4f();
        Matrix4f view = mCamera.generateViewMat();
        sortedVeges.sort((vector3f, t1) -> {
            temp1.set(vector3f, 1.0f).mul(view);
            temp2.set(t1, 1.0f).mul(view);

            temp1.w = temp2.w = 0.f;
            float dis1 = temp1.distance(0.f,0.f,0.f,0.f);
            float dis2 = temp2.distance(0.f,0.f,0.f,0.f);
            return Float.compare(dis1, dis2);
        });

        Collections.reverse(sortedVeges);

        for( Vector3f vege : sortedVeges)   {
            mModel.setTranslation(vege);
            mvpManager.setValue("model", mModel);
            glDrawArrays(GL_TRIANGLES, 0, 6);
        }
    ...
    public static void main(String[] args)  {
        OGLApplicationAbstract application = new BlendTesting2();
        application.run();
    }
}
