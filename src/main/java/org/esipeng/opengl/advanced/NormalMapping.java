...
public class NormalMapping extends OGLApplicationGL33 {
    private static final Logger logger = LoggerFactory.getLogger(NormalMapping.class);
    ...
    private int mDiffuseMap, mNormalMap;
    ...
    protected boolean applicationInitAfterContext() {
        ...
        try {
            mShader = compileAndLinkProgram(
                    "advanced/normalMapping/vertex.glsl",
                    "advanced/normalMapping/fragment.glsl"
            );

            mDiffuseMap = loadTextureFromResource("advanced/normalMapping/brickwall.jpg");
            mNormalMap = loadTextureFromResource("advanced/normalMapping/brickwall_normal.jpg");
        } catch (Exception e)   {
            e.printStackTrace();
            return false;
        }
        ...
        glfwSetKeyCallback(mWindow, new GLFWKeyCallback() {
            ...
            public void invoke(long window, int key, int scancode, int action, int mods) {
                if(key == GLFW_KEY_B && action == GLFW_RELEASE) {
                    NormalMapping.this.enableNormalMap = !NormalMapping.this.enableNormalMap;
                }

                if(enableNormalMap)
                    setUniform1i(mShader,"enableNormalMap", 1);
                else
                    setUniform1i(mShader,"enableNormalMap", 0);
            }
        });
        return true;
    }
    ...
    protected void draw() {
        ...
        mMatTemp.identity().rotate((float)glfwGetTime() * 0.3f,mRotateAxis);
    ...
    public static void main(String[] args)  {
        OGLApplicationAbstract application = new NormalMapping();
        application.run();
    }
}
